...
\item[$\Ra$] On $D$ we define a relation: $x \stackrel{p}\sim y$ if $\exists \gamma: [0,1] \ra D$ continuous such that $\gamma(0) = x$ and $\gamma(1) = y$.
\begin{itemize}
...
\item[$\bullet$] If $D$ is not pathwise connected then there exist at least two different cosets w.r.t. $\stackrel{p}\sim$, hence $D$ is disconnected.
\end{itemize}
\end{itemize}
\end{proof}
\end{itemize}
